...
export default function Preferences() {
  ...
  usePageTitle('Rule34 Infinite - Настройки')

  return (
    <FlexColumn>
      <Header />
      <DefaultPageColumn>
        <TitledSurface title='Общее'>
          <PrefPreloadVideos />
          <PrefLoadOriginals />
          <PrefTagSuggestions />
          <PrefShowPostDetails />
        ...
        <TitledSurface title='Экспериментальное'>
          <Faded>
            Эти функции все еще находятся в стадии активной разработки. В будущем они могут сильно измениться.
          </Faded>
          <HorizontalLine />
          <PrefAutoPlay />
        </TitledSurface>
        <TitledSurface title='Для разработчиков'>
          <PrefMetadata />
          <PrefBackends />
          <FeatureDetection />
          <ResetResultsButton />
